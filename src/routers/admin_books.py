...
def confirm_book_import(req: AdminConfirmBookImportRequest, authorization: Optional[str] = Header(None)):
    ...
    try:
        book_folder = book_service.imports.import_book(temp_path, target_path, title=req.title, author=req.author, language=req.language, start_node=req.start_node, tier_id=req.tier_id, narrator_id=req.narrator_id, generate_audios=req.generate_audios, tts_engine=req.tts_engine, voice_name=req.voice_name)
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
